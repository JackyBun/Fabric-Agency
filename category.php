						...
						<table id="example1" class="table table-bordered table-hover table-striped">
							<thead class="thead-dark">
								<tr>
									...
									<th width="100">Bolt</th>
									<th width="100">Supplier Name</th>
									<th width="100">Phone Number</th>
									<th width="100">Quantity Warehouse</th>
									<th width="160">Current Price</th>
									<th width="60">Quantity Imported</th>
									<th width="160">Purchase Price</th>
									<th width="100">Bolt Ordered</th>
								</tr>
							</thead>
							<tbody>
								<?php
								...
								while ($row_product = mysqli_fetch_array($product)) {
									$fab_ID = $row_product['Fabric_Code'];
								?>
									<tr>
										<td>
											<?php
											echo $row_product['Fabric_Code'];
											?>
										</td>
										<td>
											<?php
										...
										<td>
											<?php
											echo $row_product['Color'];
											?>
										</td>
										<td>
										<?php
											$bolt_ID = mysqli_query($conn, "SELECT * FROM bolt WHERE Fabric_Code = $fab_ID");
											while ($row_bolt_ID = mysqli_fetch_array($bolt_ID)) {
												if($row_bolt_ID['Order_Code'] == NULL) {
												echo "<b>ID:</b> " . $row_bolt_ID['Bolt_Code'];
												echo "<br>";
												echo "<b>Length:</b> " . $row_bolt_ID['Length'];
												echo "<br>";
											?>
												<br>
											<?php
										...
										<td>
										<?php
											$sup_INF = mysqli_query($conn, "SELECT * FROM imported WHERE Fabric_Code = $fab_ID");
											while ($row_sup_INF = mysqli_fetch_array($sup_INF)) {
												$sup_ID = $row_sup_INF['Supplier_Code'];
												$sup_Name = mysqli_query($conn, "SELECT * FROM supplier WHERE Supplier_Code = $sup_ID");
												$row_sup_Name = mysqli_fetch_array($sup_Name);
												echo $row_sup_Name['Sup_Name'];
											?>
										</td>
										<td>
											<?php
											$sup_Phone = mysqli_query($conn,"SELECT * FROM supplier_phone WHERE Supplier_Code = $sup_ID");
											while($row_sup_Phone = mysqli_fetch_array($sup_Phone)){
												echo $row_sup_Phone['Sup_phone_number'];
												echo "<br>";
											?>
											<br>
											<?php
										...
										<td>
											<b>
												<?php
												$cur_Price = mysqli_query($conn, "SELECT * FROM current_price WHERE Fabric_Code = $fab_ID");
												$row_cur_Price = mysqli_fetch_array($cur_Price);
												echo $row_cur_Price['Price'];
												?>
											</b>
											<br><b>Date:</b> 
											<?php
											echo $row_cur_Price['Date_of_price'];
											?>
										</td>
										<td>
											<?php
											echo $row_sup_INF['Qty_of_cate'];
											?>
										</td>
										<td>
											<b>
												<?php
												echo $row_sup_INF['Purchase_price'];
												?>
											</b>
											<br><b>Date:</b>  
											<?php
											echo $row_sup_INF['Date'];
											?>
										</td>
										<td>
										<?php
											...
											while ($row_bolt_ID = mysqli_fetch_array($bolt_ID)) {
												if($row_bolt_ID['Order_Code'] != NULL) {
												echo "<b>Order:</b> " . $row_bolt_ID['Order_Code'];
												echo "<br>";
												echo "<b>ID:</b> " . $row_bolt_ID['Bolt_Code'];
												echo "<br>";
												echo "<b>Length:</b> " . $row_bolt_ID['Length'];
