...
if (isset($_POST['form1'])) {
	$sup_Name = $_POST['p_supname'];
	$sup_Address = $_POST['p_address'];
	$sup_Bank = $_POST['p_bankacc'];
	$sup_Tax = $_POST['p_taxcode'];
	$sup_Phone = $_POST['p_phone'];
	$emp_ID = $_POST['p_employeeid'];

	$reset = mysqli_query($conn, "ALTER TABLE supplier AUTO_INCREMENT 1");
	mysqli_query($conn, "INSERT INTO supplier(Sup_Name,Tax_code,Sup_Address,Bank_account,Partner_staff_Code) VALUES ('$sup_Name',$sup_Tax,'$sup_Address',$sup_Bank,$emp_ID)");
	$sup_ID = mysqli_insert_id($conn);
	for ($i = 0; $i < COUNT($sup_Phone); $i++)
		mysqli_query($conn, "INSERT INTO supplier_phone VALUES ($sup_ID,'$sup_Phone[$i]')");
?>
	<script>
	alert("YOUR PROCESS HAD DONE!");
					...
					<div class="box box-info">
						<div class="box-body">
							<div class="form-group">
								...
								<div class="col-sm-4">
									<input type="text" name="p_supname" class="form-control" required>
								</div>
							</div>
							<div class="form-group">
							...
							<div class="form-group">
								<label for="" class="col-sm-3 control-label">Bank Account <span>*</span></label>
								<div class="col-sm-4">
									<input type="text" name="p_bankacc" class="form-control" required>
								</div>
							</div>
							<div class="form-group">
								<label for="" class="col-sm-3 control-label">Tax Code <span>*</span></label>
								<div class="col-sm-4">
									<input type="text" name="p_taxcode" class="form-control" required>
								</div>
							</div>
							<div class="form-group">
											...
											<tr>
												<td>
													<div class="upload-btn">
														<input type="text" name="p_phone[]" style="margin-bottom:5px;" required>
													</div>
												</td>
								...
								<div class="col-sm-2">
									<input type="button" id="btnAddNew" value="Add Phone" style="margin-top: 5px;margin-bottom:10px;border:0;color: #fff;font-size: 14px;border-radius:3px;" class="btn btn-warning btn-xs">
								</div>
							</div>
							<div class="form-group">
								<label for="" class="col-sm-3 control-label">Employee ID <span>*</span></label>
								<div class="col-sm-4">
									<input type="text" name="p_employeeid" class="form-control" required>
								</div>
							</div>
							<div class="form-group">
								<label for="" class="col-sm-3 control-label"></label>
								<div class="col-sm-6">
									<button type="submit" class="btn btn-success pull-left" name="form1">Add Supplier</button>
								</div>
							</div>
						</div>
					</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#btnAddNew").click(function() {
			...
			var addLink = "<div class=\"upload-btn" + rowNumber + "\"><input type=\"text\" name=\"p_phone[]\"  style=\"margin-bottom:5px;\"></div>";
